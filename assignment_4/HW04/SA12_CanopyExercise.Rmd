...
date: "24/02/2021 updated `r format(Sys.time(), '%B %d, %Y')`"
output:
...
# Create histograms of the total count
hist(tree_counts_renamed$tree_cnt,
     main = "Histogram of Trees in manhattan",
     xlab = "Tree counts",
     ylab = "Frequency")
```
...
neighborhoods_counts <- mutate(neighborhoods_counts, 
                            tree_cnt = ifelse(is.na(tree_cnt), 
                                              0, tree_cnt)) #If tree cnt = NA then tree cnt = 0 ELSE just use tree cnt
...
#bind columns to neighborhoods (This is done, because I'm lazy, so I don't have to use different objects)
neighborhoods <- neighborhoods %>% 
  cbind(avg_canopy = c(neighborhoods_avg_canopy$avg_canopy)) %>% 
  cbind(tree_density = c(neighborhoods_counts$tree_density))
head(neighborhoods)
...
ggplot(neighborhoods, aes(x = tree_density, y = avg_canopy)) + 
    geom_point() + 
    stat_smooth() + 
    labs(title = "tree density vs average canopy",
         x = "tree density",
         y = "average canopy")
...
#Do crs match?
crs(tree_counts)
crs(neighborhoods_avg_canopy)


# Simplify name
#neighborhoods <- neighborhoods_avg_canopy 
#I already did this on line 152
...
tm_shape(neighborhoods, bbox = manhattan) + 
...
#Create map and place neighborhoods on top of manhattan raster
...
#4  Create the second map of tree measures with `tm_shape()` and `tm_polygons()` 
map2 <- tm_shape(neighborhoods, bbox = manhattan) + 
    tm_polygons(c("tree_density", "avg_canopy"),
                palette = "Greens", 
                style = "quantile", n = 7, 
                title = c("Trees per sq. KM", "Average tree canopy (%)"))
